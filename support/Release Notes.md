**Combined with the README.md doc on 2026-03-24**
...
<tr><td><strong>Tap-to-restore when auto-dimmed</strong></td><td><code>backlightCancelDim()</code> added to <code>hal/backlight.h/.cpp</code>. Any touch while the screen is dimmed restores <code>preDimBrightness</code>, clears the dim flag, and calls <code>backlightSet()</code> before the touch is passed to <code>handleTouch()</code> — the waking tap is still processed normally.</td></tr>
<tr><td><strong>Minimum brightness floor</strong></td><td><code>BRIGHT_MIN = 30</code> (~12%) added to <code>constants.h</code>. Brightness slider lower bound raised from 0 → <code>BRIGHT_MIN</code>. Saved preference clamped to <code>BRIGHT_MIN</code> on boot so a previously stored low value cannot leave the display inaccessible.</td></tr>
<tr><td><strong>autoDimLevel capped at normal brightness</strong></td><td>Auto-dim level <code>+</code> button and brightness slider both enforce <code>autoDimLevel ≤ brightPct</code> (= <code>brightness × 100 / 255</code>). Prevents the dim level from being set higher than normal brightness, which would cause auto-dim to brighten the screen.</td></tr>
...
<tr><td><strong>DST toggle (MANUAL timezone mode)</strong></td><td>New <code>bool manualDstActive</code> (NVS key <code>"manualDst"</code>) on the Regional Setup screen in MANUAL mode. Displayed as a <strong>DST: OFF / DST: ON</strong> button (outlined when off, filled orange when on) beside the +/− offset buttons. Tapping adjusts only the effective POSIX offset string (<code>gmtOffset_sec ± 3600</code>) and persists to NVS. Selecting a city from the embedded table resets <code>manualDstActive = false</code> — city-derived POSIX rules handle DST automatically.</td></tr>
